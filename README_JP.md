...
デフォルトの `start` は Electron overlay を優先します。
Linux では Electron が使えない場合に GTK へフォールバックします。
従来のブラウザ表示を使いたい場合だけ `--browser` を付けてください。

## 🪟 Electron Overlay

Windows / macOS 対応の本命は Electron です。
Linux でも GTK の代替 backend として使えます。
...
- `auto`
  Electron を優先し、Linux では次に GTK、最後に browser
- `gtk`
...
pet-companion start --overlay-backend electron     # Electron を強制
